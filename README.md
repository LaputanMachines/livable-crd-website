...
**Publication is one switch, and it has been on since the release on September 22, 2026.** `PUBLISH_GRADES` at the top of [`scripts/sync-questionnaire.py`](scripts/sync-questionnaire.py) decides: while it is `False`, no grade and no free-text answer is written to `scores.yml` at all, so grading in progress cannot reach a public page. It was flipped to `True` in one commit on the release date, and each run since publishes every candidate and every subject at once; flipping it back removes them just as completely. To see what a release would produce without committing anything, run `PUBLISH_GRADES=1 python3 scripts/sync-questionnaire.py --dry-run`.

**Each question's methodology has its own switch, and it is off.** `PUBLISH_METHODOLOGY`, beside `PUBLISH_GRADES`, decides whether the Question Registry's `Methodology` column (how each question is scored) is written to `questions.yml` and shown on /questionnaire/. It is held back separately because it describes the questions rather than the answers: while any candidate is still filling in the questionnaire, publishing it would tell them how to score well. `PUBLISH_METHODOLOGY=1` or `--publish-methodology` overrides it for one run.

**A publishing run stops if a grading tab has hidden rows.** The sheet is read through gviz, which returns only the rows a filter leaves visible and gives no sign that anything is missing. A filter left on `Grade - Housing` has twice made every questionnaire score on it look blank. So before reading results, the sync downloads the link-readable xlsx export and checks `Category Grades` and every `Grade - <Subject>` tab for hidden rows. If it finds any, it names the tab and writes nothing. The fix is **Data → Remove filter** on that tab.
